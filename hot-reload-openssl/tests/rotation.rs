...
use hot_reload_openssl::{Event, Watcher};
use openssl::{
    asn1::Asn1Time,
    bn::BigNum,
    hash::MessageDigest,
...
fn start(cert: &Path, key: &Path, scenario: &str) -> (Server, Watcher, mpsc::Receiver<Event>) {
    ...
    let thread = thread::spawn(move || {
        ...
        let (builder, mut watcher) = if defaults {
            hot_reload_openssl::Builder::new(cert, key).build().unwrap()
        } else {
            hot_reload_openssl::Builder::new(cert, key)
